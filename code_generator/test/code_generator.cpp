...
void CodeGenerator::parseTFModel(const tflite::Model *tf_model)
{
    ...
    for(size_t i = 0; !flag && i < subgraph->operators()->size(); ++i)
    {
        ...
        case tflite::BuiltinOperator_DEPTHWISE_CONV_2D:
        {
            ...
            auto bias_buffer = tf_model->buffers()->Get(bias_tensor->buffer());
            const float* bias_data = reinterpret_cast<const float*>(filter_buffer->data()->data());
            size_t bias_buffer_size = bias_buffer->data()->size() / sizeof(float);
            std::vector<float> flat_bias_buffer(bias_buffer_size);
...
void print_padded_channel_to_file(std::ofstream &out_file, float *data, int width, int height) {
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            out_file << static_cast<float>(data[i * width + j]) << " ";
        }
        out_file << std::endl;
    }
}
...
)
{
    ...
    const int kernel_depth = input_ch;

    std::cout << "kernel detph is : " << kernel_depth << std::endl;
    ...
    for (int oc = 0; oc < output_channels; ++oc) { 
        for (int oy = 0; oy < output_height; ++oy) { 
            for (int ox = 0; ox < output_width; ++ox) { 
                    ...
                    for (int ky = 0; ky < kernel_height; ++ky) { 
                        for (int kx = 0; kx < kernel_width; ++kx) { 
                            ...
                            if (ix < input_x && iy < input_y) {
                                ...
                                int kernel_index = ((oc * kernel_depth * kernel_height * kernel_width) +
                ...
                acc += bias[oc];
                if(relu_flag)acc = std::max(output_activation_min, std::min(acc, output_activation_max)); // ReLU6
    ...
    std::cout << "conv2d ouptut indexing..." << std::endl;
    for(int j = 0; j < 10; j++)
    {
        std::cout << output[j] << " ";
    }
...
)
{
    ...
    for (int oc = 0; oc < output_channels; ++oc) {  // Output (and input) channels loop
        for (int oy = 0; oy < output_height; ++oy) {  // Output height loop
            for (int ox = 0; ox < output_width; ++ox) {  // Output width loop
                float acc = 0.0f;
                for (int ky = 0; ky < kernel_height; ++ky) {  // Kernel height loop
                    for (int kx = 0; kx < kernel_width; ++kx) {  // Kernel width loop
                        int ix = ox * stride_value + kx;
                        int iy = oy * stride_value + ky;
                        if (ix < input_x && iy < input_y) {
                            int input_index = (iy * input_x + ix) * input_ch + oc;  // Use `oc` for depthwise conv
                            int kernel_index = (oc * kernel_height * kernel_width) + (ky * kernel_width + kx);
                            acc += input[input_index] * kernel[kernel_index];
                        }
                    }
                }
    ...
    std::cout << "depthwise ouptut indexing..." << std::endl;
    for(int j = 0; j < 10; j++)
    {
        std::cout << output[j] << " ";
    }
        ...
        for (int oy = 0; oy < output_height; ++oy) {
            for (int ox = 0; ox < output_width; ++ox) {
                ...
                if (oy < paddings[2] || oy >= (output_height - paddings[3]) ||
                    ...
                    // 패딩 영역
                    output[output_index] = static_cast<float>(pad_value);
                } else {
                    // 원본 데이터 영역
